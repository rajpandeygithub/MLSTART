class ReportGenerator:
    ...
    def generate_report(self, model_metrics, best_model_name, rankings):
        ...
        for metric in next(iter(model_metrics.values())).keys():
            report_lines.append(f"\n{metric} ({'Higher' if metric != 'MAE' and metric != 'MSE' and metric != 'RMSE' else 'Lower'} is Better)")
            report_lines.append("-" * 50)
        ...
        for model_name in rankings.keys():
            ...
            row = "{:<25} {:<15.2f}".format(model_name, avg_rank)
            report_lines.append(row)

        # Recommendation
        report_lines.append("=" * 50)
        report_lines.append(f"\nBest Model: {best_model_name}")
        report_lines.append(f"Based on the evaluation, {best_model_name} is the top-performing model for your dataset. We recommend starting with this model for your task.")
